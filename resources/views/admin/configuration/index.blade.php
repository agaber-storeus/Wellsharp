                ...
                <div class="table-wrap"><table class="table"><thead><tr><th class="configuration-order-heading">Order</th><th>Name</th><th>Status</th><th>Actions</th></tr></thead><tbody>
                    <template x-for="row in section.rows" :key="row.id">
                        <tr x-bind:class="dragging && dragging.id === row.id ? 'configuration-row-dragging' : ''" x-on:dragover.prevent="dragOver(section, row, $event)" x-on:drop="drop(section, row, $event)">
                            ...
                            <td><input class="configuration-name-input" x-model="row.name" required x-on:keydown.enter.prevent="save(section, row)"></td>
                            ...
                            <td>
                                <div class="actions configuration-actions">
                                    <form method="POST" x-bind:action="row.update_url" x-on:submit.prevent="save(section, row)">
                                        @csrf
                                        @method('PATCH')
                                        <input type="hidden" name="name" x-bind:value="row.name">
                                        <button class="btn secondary small" type="submit" x-on:click.stop.prevent="save(section, row)" x-bind:disabled="row.saving" x-text="row.saving ? 'Saving...' : 'Save'"></button>
                                    </form>
                                    <form method="POST" x-bind:action="row.toggle_url" x-on:submit.prevent="toggle(section, row)">
                                        @csrf
                                        @method('PATCH')
                                        <button class="btn secondary small" type="submit" x-on:click.stop.prevent="toggle(section, row)" x-bind:disabled="row.toggling" x-text="row.toggling ? 'Updating...' : (row.active ? 'Deactivate' : 'Activate')"></button>
                                    </form>
                                </div>
                            </td>
                        </tr>
...
<style>
    .configuration-sections{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:24px}.configuration-section{margin-top:0;border:1px solid var(--admin-line);border-radius:7px;padding:18px;background:#fff}.configuration-create{display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin:12px 0 16px}.configuration-create input:first-of-type{flex:1;min-width:160px}.configuration-create input,.configuration-name-input{border:1px solid #b9c7d2;border-radius:5px;padding:9px 10px;background:#fff;color:var(--admin-ink);font:inherit}.configuration-order-heading{width:64px}.configuration-order-cell{text-align:center!important}.configuration-drag-handle{display:inline-grid;place-items:center;width:30px;height:30px;border:1px solid var(--admin-line);border-radius:5px;background:#f5f8fa;color:var(--admin-blue);font:700 16px/1 Arial,sans-serif;letter-spacing:-2px;cursor:grab;user-select:none}.configuration-drag-handle:active{cursor:grabbing}.configuration-name-input{width:100%;min-width:0}.configuration-section .table td{vertical-align:middle}.configuration-section .table td:last-child{white-space:nowrap}.configuration-actions{position:relative;z-index:2}.configuration-actions form{display:inline-flex;margin:0}.configuration-actions .btn{cursor:pointer!important;pointer-events:auto!important;touch-action:manipulation}.configuration-actions .btn:disabled{cursor:wait!important}.configuration-row-dragging{opacity:.45;background:#eef6fb!important}.configuration-section .btn:disabled{opacity:.6}@media(max-width:900px){.configuration-sections{grid-template-columns:1fr}}
</style>

<script>
    window.configurationTable = function (initialSections) {
        return {
            sections: (initialSections || []).map((section) => ({ ...section, rows: section.rows || [], newName: '', saving: false, ordering: false })),
            dragging: null,
            notice: '',
            noticeType: 'success',
            ...
            async request(url, method, body = {}) {
                const response = await fetch(url, { method, headers: { 'Accept': 'application/json', 'Content-Type': 'application/json', 'X-CSRF-TOKEN': @js(csrf_token()), 'X-Requested-With': 'XMLHttpRequest' }, body: JSON.stringify(body) });
            ...
            async create(section) {
                ...
                if (!name) return;
                section.saving = true;
                try { const payload = await this.request(section.store_url, 'POST', { name }); section.rows.unshift(payload.row); section.newName = ''; this.notify(payload.message); }
            ...
            async save(section, row) {
                ...
                row.saving = true;
                try { const payload = await this.request(row.update_url, 'PATCH', { name }); Object.assign(row, payload.row); this.notify(payload.message); }
                catch (error) { this.notify(error.message, 'error'); }
                finally { row.saving = false; }
            },
            async toggle(section, row) {
                row.toggling = true;
                try { const payload = await this.request(row.toggle_url, 'PATCH'); Object.assign(row, payload.row); this.notify(payload.message); }
                catch (error) { this.notify(error.message, 'error'); }
            ...
            async drop(section, row, event) {
                ...
                try { const payload = await this.request(section.reorder_url, 'PATCH', { order: section.rows.map((item) => item.id) }); section.rows = payload.rows; this.notify(payload.message); }
