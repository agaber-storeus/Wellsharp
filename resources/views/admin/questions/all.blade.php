...
<style>
    ...
    .question-row-error { display: block; color: #b42318; font-size: 12px; margin-top: 5px; }
</style>
            ...
            <tbody>
                <template x-for="question in rows" :key="question.id">
                    <tr>
                        ...
                        <td><div class="admin-actions-cell"><a x-bind:href="question.edit_url">Edit</a><template x-if="question.active"><button class="btn secondary small question-status-toggle" type="button" x-on:click.prevent="archiveQuestion(question)" x-bind:disabled="question.archiving" x-bind:aria-busy="question.archiving ? 'true' : 'false'" x-text="question.archiving ? 'Updating...' : 'Archive'"></button></template><template x-if="!question.active"><button class="btn secondary small question-status-toggle" type="button" x-on:click.prevent="unarchiveQuestion(question)" x-bind:disabled="question.archiving" x-bind:aria-busy="question.archiving ? 'true' : 'false'" x-text="question.archiving ? 'Updating...' : 'Unarchive'"></button></template><span class="question-row-error" x-show="question.rowError" x-text="question.rowError" x-cloak></span></div></td>
                    </tr>
                </template>
...
<script>
    window.questionTable = function (endpoint, initialRows, initialMeta) {
        return {
            endpoint, rows: (initialRows || []).map(question => ({ ...question, archiving: false, rowError: '' })), meta: initialMeta || { current_page: 1, last_page: 1, total: 0, from: null, to: null }, search: @js($search), courseId: @js((string) $courseId), type: @js($type), difficulty: @js($difficulty), status: @js($status), sort: 'created_at', direction: 'desc', loading: false, error: '', request: null,
            ...
            async load(page) { page = Math.max(1, page || 1); if (this.request) this.request.abort(); this.request = new AbortController(); const params = new URLSearchParams({ page, sort: this.sort, direction: this.direction }); if (this.search) params.set('search', this.search); if (this.courseId) params.set('course_id', this.courseId); if (this.type) params.set('type', this.type); if (this.difficulty) params.set('difficulty', this.difficulty); if (this.status) params.set('status', this.status); this.loading = true; this.error = ''; try { const response = await fetch(this.endpoint + '?' + params.toString(), { headers: { Accept: 'application/json' }, signal: this.request.signal }); if (!response.ok) throw new Error('Question search failed'); const payload = await response.json(); this.rows = (payload.data || []).map(question => ({ ...question, archiving: false, rowError: '' })); this.meta = payload.meta; } catch (error) { if (error.name !== 'AbortError') this.error = 'Questions could not be loaded. Try again.'; } finally { this.loading = false; } },
            async archiveQuestion(question) { if (!question.active || question.archiving) return; await this.changeArchiveState(question, question.archive_url, 'Question could not be archived.', false, 'DELETE'); },
            async unarchiveQuestion(question) { if (question.active || question.archiving) return; await this.changeArchiveState(question, question.unarchive_url, 'Question could not be unarchived.', true, 'PATCH'); },
            async changeArchiveState(question, url, errorMessage, active, method) { question.archiving = true; question.rowError = ''; this.error = ''; try { const response = await fetch(url, { method, credentials: 'same-origin', headers: { Accept: 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content || '', 'X-Requested-With': 'XMLHttpRequest' } }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.message || errorMessage); question.active = active; question.status_label = payload.status_label || (active ? 'Active' : 'Archived'); } catch (exception) { question.rowError = exception.message || errorMessage; this.error = question.rowError; } finally { question.archiving = false; } }
        };
    };
</script>
@endsection
