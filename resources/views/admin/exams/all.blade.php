...
<style>[x-cloak]{display:none!important}.exam-table-loading{opacity:.55;pointer-events:none}.exam-sort{border:0;background:transparent;color:inherit;font:inherit;font-weight:600;cursor:pointer;padding:0}.exam-sort:hover{color:var(--admin-blue)}.exam-sort-icon{font-size:11px;margin-left:3px}.exam-error{background:#fef3f2;color:#b42318;border-radius:5px;padding:10px 12px;margin-bottom:15px}</style>
    ...
    window.examTable = function (endpoint, initialRows, initialMeta) {
        return {
            ...
            async load(page) {
                ...
                if (this.search) params.set('search', this.search); if (this.courseId) params.set('course_id', this.courseId); if (this.status) params.set('status', this.status);
                this.loading = true; this.error = '';
                try { const response = await fetch(this.endpoint + '?' + params.toString(), { headers: { Accept: 'application/json' }, signal: this.request.signal }); if (!response.ok) throw new Error('Exam search failed'); const payload = await response.json(); this.rows = payload.data; this.meta = payload.meta; }
                catch (error) { if (error.name !== 'AbortError') this.error = 'Exams could not be loaded. Try again.'; }
