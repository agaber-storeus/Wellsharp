...
**BR-009** — `ExamClassSynchronizer` matches an existing Class by `course_id` + exact date match on `starts_at`/`ends_at` before creating a new one; multiple schedules for the same course on the same calendar days share one Class.
Source: `app/Services/ExamClassSynchronizer::sync()`.
...
**BR-011** — Automatic (scheduler-driven) start only fires once `starts_at <= now()`; automatic end only fires once `ends_at <= now()`. Manual start/end by staff can happen **before** the configured time (early start/end is allowed for humans, not for the scheduler).
...
**BR-014** — The scheduler command `wellsharp:process-exam-schedules` (intended to run every minute) drives fully automatic start/end using the exact same code path as manual staff control, just with `source='automatic'` and no actor attributed.
...
**BR-016** — Absent a manual early-start override (`override_started_at`), a student can only start within the schedule's `start_date`–`end_date` window (`start_date` future → blocked; `end_date` end-of-day past → blocked).
...
**BR-030** — Every issued certificate gets exactly 3 `CertificateDocument` rows (Knowledge Assessment Report, Completion Card Front, Completion Card Back), created via `firstOrCreate` keyed on `(certificate_id, type)`.
