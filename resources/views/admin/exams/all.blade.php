...
<style>
    ...
    .exam-row-error { display: block; color: #b42318; font-size: 12px; margin-top: 5px; }
...
<script>
    window.examTable = function (endpoint, initialRows, initialMeta) {
        return {
            endpoint, rows: (initialRows || []).map(exam => ({ ...exam, archiving: false, rowError: '' })), meta: initialMeta || { current_page: 1, last_page: 1, total: 0, from: null, to: null }, search: @js($search), courseId: @js((string) $courseId), status: @js($status), sort: 'created_at', direction: 'desc', loading: false, error: '', request: null,
            ...
            async changeArchiveState(exam, url, errorMessage, state) { exam.archiving = true; exam.rowError = ''; this.error = ''; try { const response = await fetch(url, { method: 'PATCH', credentials: 'same-origin', headers: { Accept: 'application/json', 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content || '', 'X-Requested-With': 'XMLHttpRequest' }, body: '{}' }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.message || errorMessage); exam.status = payload.status || state; exam.status_label = payload.status_label || (state === 'draft' ? 'Draft' : 'Archived'); exam.can_archive = state !== 'archived'; exam.can_unarchive = state === 'archived'; } catch (exception) { exam.rowError = exception.message || errorMessage; this.error = exam.rowError; } finally { exam.archiving = false; } }
        };
    };
</script>
@endsection
