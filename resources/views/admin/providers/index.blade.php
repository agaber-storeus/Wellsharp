                    ...
                    <tr>
                        ...
                        <td>
                            <div class="provider-status-cell">
                                ...
                                <template x-if="provider.status !== 'archived'"><button class="btn secondary small provider-status-toggle" type="button" x-on:click.prevent="toggleStatus(provider)" x-bind:aria-busy="provider.saving ? 'true' : 'false'" x-text="provider.saving ? 'Updating...' : (provider.status === 'active' ? 'Deactivate' : 'Activate')"></button></template>
                            </div>
                        </td>
...
<script>
    window.providerTable = function (endpoint, initialRows, initialMeta) {
        return {
            ...
            async archiveProvider(provider) { if (provider.status === 'archived' || provider.saving || provider.archiving) return; provider.archiving = true; this.error = ''; try { const response = await fetch(provider.archive_url, { method: 'PATCH', credentials: 'same-origin', headers: { Accept: 'application/json', 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content || '', 'X-Requested-With': 'XMLHttpRequest' } }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.message || 'Provider could not be archived.'); provider.saved_status = payload.status; provider.status = payload.status; provider.status_label = payload.status_label; } catch (exception) { this.error = exception.message || 'Provider could not be archived.'; } finally { provider.archiving = false; } }
        };
    };
</script>
@endsection
