...
- **Deliberate exception, added 2026-08-16/17**: Student (only) passwords are additionally stored as an app-key-encrypted, reversible copy (`users.password_ciphertext`), separate from the hash used for actual login, so Admin/Proctor/Instructor can look up and hand a Student their password (no self-service reset flow exists). Gated by `UserPolicy::viewPassword()`, audited on every reveal (`student.password_viewed`). See BUSINESS_RULES.md BR-037..BR-042 — this is the one place in the system where a password-shaped value is ever recoverable, and it was scoped narrowly (Student targets only, staff-only viewers, audited) specifically to avoid the much larger risk of a plaintext-password column.
