...
<script>
    window.courseQuestionTable = function (endpoint, initialRows, initialMeta) {
        return {
            endpoint, rows: (initialRows || []).map(question => ({ ...question, archiving: false, rowError: '' })), meta: initialMeta || { current_page: 1, last_page: 1, total: 0, from: null, to: null }, search: @js($search), difficulty: @js($difficulty), type: @js($type), status: @js($status), sort: 'created_at', direction: 'desc', loading: false, error: '', request: null,
            ...
            async changeArchiveState(question, url, errorMessage, active, method) { question.archiving = true; question.rowError = ''; try { const response = await fetch(url, { method, credentials: 'same-origin', headers: { Accept: 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content || '', 'X-Requested-With': 'XMLHttpRequest' } }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.message || errorMessage); question.active = active; } catch (exception) { question.rowError = exception.message || errorMessage; } finally { question.archiving = false; } }
        };
    };
</script>
@endsection
