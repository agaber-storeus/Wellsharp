...
<section class="workspace">
  ...
  <section class="center-actions student-continue-panel">
    @if($studentFlowSchedule)
      <form method="POST" action="{{ route('student.confirm.store', $studentFlowSchedule) }}">
        @csrf
        <button class="green-btn arrow" type="submit">Continue</button>
      </form>
    @else
      <button class="green-btn arrow" type="button" disabled>Continue</button>
      <p class="muted">There is no scheduled class available for your account.</p>
    @endif
  </section>
</section>
@endsection
